...
def remove_unsupported_dtypes(
    state_keys: list[str], state_shapes: dict[str, tuple], state_dtypes: dict[str, str]
) -> tuple[list[str], dict[str, tuple], dict[str, str]]:
    new_state_keys = []
    new_state_shapes = {}
    new_state_dtypes = {}
...
def discover_dataset_properties(hdf5_paths: list[str]) -> dict[str, Any]:
    ...
    with h5py.File(hdf5_path, "r") as f:
                ...
                if isinstance(item, h5py.Group):
                    ...
                    if key == "obs":
                        ...
                        for obs_key in obs_group:
                            if obs_key.endswith("_rgb"):
                                ...
                                state_keys.add(obs_key)
                                if obs_key not in state_shapes:
                                    state_shapes[obs_key] = obs_group[obs_key].shape[
                                        1:
                                    ]  # Skip time dimension
                                    state_dtypes[obs_key] = obs_group[obs_key].dtype
                    else:
                        print(
                            colored(
                                f"Warning: Other group {key} found in episode {ep_name}, but not handled.",
                                "yellow",
                            )
                        )
                    ...
                    if key not in ["actions"] and key not in other_keys and key in max_dims:
                        ...
                        other_keys[key] = (
                            ...
                            if len(item.shape) > 1
                            else ((1,), str(item.dtype))
                        )

        # remove keys whose dtype are dobjects
        state_keys, state_shapes, state_dtypes = remove_unsupported_dtypes(
            state_keys, state_shapes, state_dtypes
        )
        ...
        if "data" in f and "env_args" in f["data"].attrs:
            ...
            if env_args:
                ...
                if "control_freq" not in env_args:
                    print(
                        colored(
                            f"Warning: control_freq not found in env_args for {hdf5_path}. Defaulting to 20 fps.",
                            "red",
                        )
                    )
        ...
        return {
            ...
            "state_keys": sorted(state_keys),  # Sort for consistency
...
def load_robocasa_episode(f: h5py.File, ep_name: str, other_keys: dict[str, tuple] = None) -> dict[str, Any]:
    ...
    ep_attrs = {}
    for attr_name in ep_group.attrs:
                    ...
                    for t in range(num_steps):
                        ...
                        for state_key in properties["state_keys"]:
                            if "obs/" + state_key in properties["max_dims"]:
                                ...
                                frame_data[state_key][: ep_data["obs"][state_key][t].shape[0]] = ep_data[
                                    "obs"
                                ][state_key][t]
                            else:
