...
"""
Finetune SmolVLA on one RoboCasa dexterous-hand CoffeePressButton dataset.

This mirrors examples/training/train_policy_casa_dex_pi05.py for the first
single-dataset smoke test before moving to mixed RoboCasa datasets. It loads
lerobot/smolvla_base, keeps the VLM mostly frozen through SmolVLA's default
train_expert_only setting, and enables the new embodiment-specific action
encoder/decoder path with a single embodiment slot.

Usage:
    python examples/training/train_policy_casa_dex_smolvla.py --experiment baseline_expert_full_ft
    python examples/training/train_policy_casa_dex_smolvla.py --experiment exp1_expert_lora
    python examples/training/train_policy_casa_dex_smolvla.py --experiment exp2_vlm_expert_lora
    python examples/training/train_policy_casa_dex_smolvla.py --experiment exp3_vlm_lora_expert_full_ft
"""
...
def make_config(experiment: str = "baseline_expert_full_ft") -> TrainPipelineConfig:
    ...
    cfg = TrainPipelineConfig(
        ...
        job_name=f"robocasa_dex_smolvla_{spec.name}",
        resume=False,
        policy=SmolVLAConfig(
            ...
            pretrained_action_proj_category=ROBOCASA_DEX_EMBODIMENT_ID,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Named single-dataset RoboCasa SmolVLA finetuning experiment to run.",
    )
